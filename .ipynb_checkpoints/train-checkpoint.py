...
def train():
    ...
    dataset_train = ParquetDataset('/root/drw-crypto-market-prediction/X_train_pca.parquet', model_config['window_size'], mode='train')
    dataset_val = ParquetDataset('/root/drw-crypto-market-prediction/X_train_pca.parquet', model_config['window_size'], mode='val')
    ...
    trainer.fit(model, train_dataloaders = DataLoader(dataset_train, batch_size=128, shuffle=True, num_workers=0, drop_last=True),
                val_dataloaders = DataLoader(dataset_val, batch_size=128, shuffle=True, num_workers=0, drop_last=True))
...
if __name__ == '__main__':
    Best_ckpt = train()
    evaluate(Best_ckpt, '/root/output6.17.csv')
