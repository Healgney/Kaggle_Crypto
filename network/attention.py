...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, padding_price_q, padding_price_k):
        ...
        q_size1 = query.size(0)  # 128
        q_size2 = query.size(1)  # 31 0r 1
        q_size3 = query.size(2)  # 2*12
        key_size1 = key.size(0)
        key_size2 = key.size(1)
        key_size3 = key.size(2)
        ##################################query#################################################
        '''
        这块是做时间序列的padding，也可以不padding
        '''
        if padding_price_q is not None:
            ...
            if self.local_context_length > 1:
                ...
                padding_q = None
        query = query.permute((1, 3, 0, 2)) # [batch_size, feature_dim, coin_num, time_len]
        ...
        ########################################### local-attention ######################################################
        local_weight_q = torch.matmul(query[:, :, self.local_context_length - 1:, :],# 未来时间31，全部时间35
                                      query.transpose(-2, -1)) / ma.sqrt(q_size3)  # [128,11,31,2*12] *[128,11,2*12,31+4]->[128,11,31,31+4]
        # [128,11,31,31+4]->[128,11,1,5*31]
        local_weight_q_list = [F.softmax(local_weight_q[:, :, i: i + 1, i: i + self.local_context_length], dim=-1) for i
                               in range(q_size2)] #进行窗口内的softmax，就不需要mask # 31 * [128, 11, 1, 5]
        local_weight_q_list = torch.cat(local_weight_q_list, 3)# 拼接softmax结果
