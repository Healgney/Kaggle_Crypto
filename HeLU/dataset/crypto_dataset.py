...
class ParquetDataset(Dataset):
    def __init__(self, parquet_file, window_size, label_col='label', device='cuda', mode='train', train_ratio=0.8,remove_nan=True):
        super().__init__()
        self.device = device
        self.mode = mode
        ...
        if self.mode == 'train':
            ...
            df = pd.read_parquet(parquet_file).iloc[:12*106+1]
        else:
